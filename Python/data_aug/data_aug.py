...
class RandomExpand(object):
    """Puts rgb image into canvas with the probability of p."""
    ...
    def __call__(self, img, bboxes):
            ...
            if random.random() < self.p:
                ...
                mean_color_values = np.array([(channels[0], channels[1], channels[2])])
                ...
                expand_image = np.zeros (
                                ...
                                dtype=img.dtype)
                expand_image[:] = mean_color_values
                expand_image[int(top):int(top + height),
                   int(left): int(left+width)] = img
                img = expand_image
                labels = bboxes[:,-1:].copy()
                bboxes = bboxes[:,:-1].copy()
                bboxes[:, :2] += (int(left), int(top))
                bboxes[:, 2:] += (int(left), int(top))
                bboxes = np.append(bboxes, labels, axis=1)

            return img, bboxes
			
class RandomCrop(object):
    """Randomly crops an image with the probability of p and crop width&height"""

    def __init__(self, p=0.5, crop_width=300, crop_height=300):
        self.p = p
    ...
    def __call__(self, img, bboxes):
            if random.random() < self.p:
                # to BoundingBox class
                bbox_list = []
                for b_list in bboxes:
                    bbox_list.append(BoundingBox(x1=b_list[0],y1=b_list[1],x2=b_list[2],y2=b_list[3],label=b_list[4]))
        
                # Define image and several bounding boxes
                image = img.copy()
                bbs = BoundingBoxesOnImage(bbox_list, shape=image.shape)
                # crop until sign is fully visible
                cropping=True
                while(cropping):
                    seq  = iaa.Sequential([
                        #iaa.PadToFixedSize(width=300, height=300),
                        iaa.CropToFixedSize(width=self.width, height=self.height)
                        ])
                    img, bbs_aug = seq(image=image, bounding_boxes=bbs)
            
                    bbs_aug_removed = bbs_aug.remove_out_of_image(fully=True, partly=False)
                    if len(bbs_aug_removed)>0:
                        count=0
                        for i, bb in enumerate(bbs_aug_removed):
                            if(not bb.is_fully_within_image(img)):
                                count+=1
                ...
                bbs_aug_clipped=bbs_aug_removed.clip_out_of_image()
                n=len(bbs_aug_clipped)
                bboxes=np.zeros(shape=(n,5))
                for i, bb in enumerate(bbs_aug_clipped):
                    bboxes[i]=(np.append(bb[:], bb.label))
